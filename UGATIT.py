...
class UGATIT(object) :
    ...
    def train(self):
        self.genA2B.train(), self.disGA.train(), self.disGB.train() ,  self.genB2A.train(), self.disLA.train(), self.disLB.train()
        ...
        # Преобразование изображения в тензор с использованием библиотеки PyTorch
        transform = transforms.Compose([
            transforms.ToTensor(),
        ])

        mask = transform(img)
        mask = torch.cat([mask, mask, mask], dim=0).unsqueeze(0)
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            # noise_level = step / self.iteration * 20
            ...
            fake_B2A = denorm(fake_B2A)
            fake_B2A = fake_B2A * mask
            fake_B2A = imagenet_norm(fake_B2A)
            fake_A2B, _, _ = self.genA2B(fake_B2A)
    ...
    def test(self):
        ...
        if not len(model_list) == 0:
            model_list.sort()
            #iter = model_list[-1].split('_')[-1].split('.')[0]
            self.load("results/training/model/training_params_latest.pt")
            print(" [*] Load SUCCESS")
        else:
            print(" [*] Load FAILURE")
            return
        ...
        #
        #     noise_level = 5
        #     noise = torch.randn_like(real_A) * noise_level
        #     real_A = real_A + noise
        ...
        for n, (real_B, _) in enumerate(self.testB_loader):
            ...
            for m, (real_A, _) in enumerate(self.testA_loader):
                image_name = os.path.basename(self.testB_loader.dataset.imgs[n][0])
                ...
                B2A = np.concatenate((RGB2BGR(tensor2numpy(denorm(cat[0]))),
                                    RGB2BGR(tensor2numpy(denorm(fake_B2A[0]))),
                                    RGB2BGR(tensor2numpy(denorm(fake_B2A2B[0])))), 0)
        ...
        # images1 = r"C:\Users\senic\PycharmProjects\BP_last\UGATIT-pytorch\dataset\training\testB"
        # images2 = r"C:\Users\senic\PycharmProjects\BP_last\UGATIT-pytorch\results\training\test"

        # # Vypočítejte KID mezi oběma sadami obrázků
        # fid_value, std_dev = fid_score.calculate_fid_given_paths([images1, images2], batch_size=64, dims=2048, device='cuda')

        # # Výsledek vypíšete
        # print("Kernel Inception Distance (KID) между двумя наборами изображений:", fid_value * 100, "+-", std_dev * 100)
